...
//extern char gateway_address[256];
extern str gateway_commonaddr;
//extern char* Evy_gatewayAddress();
...
ucontact_t* new_ucontact(str* _dom, str* _aor, str* _contact, ucontact_info_t* _ci)
{
	...
	if (gateway_commonaddr.s && gateway_commonaddr.len) {
		if (shm_str_dup( &c->common_address, &gateway_commonaddr) < 0) goto error;
...
int db_insert_ucontact(ucontact_t* _c)
{
	...
	keys[9] = &commonaddress_col;
	vals[9].type = DB1_STR;
	vals[9].nul = 0;
	vals[9].val.str_val.s = _c->common_address.s;
	vals[9].val.str_val.len = _c->common_address.len;
	nr_cols = 10;
	
	LM_DBG("Database...common_address_col = %.*s\n",vals[9].val.str_val.len,vals[9].val.str_val.s);
