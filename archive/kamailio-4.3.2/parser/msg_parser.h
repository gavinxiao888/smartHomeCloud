...
enum _custom_types_t {
	CST_ERROR_T					= -1   /*!< Error while parsing */,
	CST_OTHER_T					=  0   /*!< Some other header field */,
	CST_TOKEN_T					=  1,
	CST_CODE_T					=  2,
	CST_ID_T					=  3,
	CST_CMD_T					=  4,
	CST_EOH_T					       /*!< End of message header */
};

typedef enum _custom_types_t custom_types_t;

typedef struct custom_field {
	custom_types_t type;    /*!< Header field type */
	str name;               /*!< Header field name */
	str body;               /*!< Header field body (may not include CRLF) */
	int len;		/*!< length from hdr start until EoHF (incl.CRLF) */
	// void* parsed;           /*!< Parsed data structures */
	// struct custom_field* next; /*!< Next header field in the list */
} custom_field_t;
...
typedef struct custom_data {
	...
	struct custom_field* c_token;
	struct custom_field* c_sn_code;
	struct custom_field* c_user_id;
	struct custom_field* c_cmd;
} custom_data_t;
