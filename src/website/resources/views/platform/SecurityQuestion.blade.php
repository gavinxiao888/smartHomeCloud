...
function show_option()
{
	...
	for(var i = 0; i < option.length; i++)
	{
		...
		//option[i].style.font-family = "Microsoft YaHei";
	}
}
...
<div class="cont-head"> 
<h4><a href="/user/account">爱悠账号&nbsp;>&nbsp;</a><font style="color:#ec680c">设置密保问题</font></h4>
<h6><?php
...
<div class="form" >
<table style="margin:0 auto;font-size:14px;margin-top:20px;">
	<tr style="height:50px;">
		<td></td>
		<td>请选择不易忘记的问题进行回答，并牢记答案。<br/>安全问题将成为找回密码的重要途径。</td>
	</tr>
	<tr style="line-height:45px;">
		<td style="text-align:right">问题一:&nbsp;</td>
		<td><select class="form-control" id="select1" onFocus="reg_problem()" onBlur="show_option()">
		<?php
		$pro = '';
		if (count($problem,0)>0)
		{
			$pro = '<option >请选择密保问题</option>';
			for ($i = 0; $i < count($problem,0); $i++)
			{
				$pro .='<option id="'.$problem[$i]['id'].'" style="font-family: Microsoft YaHei;">'. $problem[$i]['problem']. '</option>';
			}
			echo $pro;
		}
		?>
	...
	<tr style="line-height:45px;">
		...
		<td><input class="form-control" type="text" style="float:right;display:block" id="a1"/></td>
	</tr>
	<tr style="line-height:45px;">
		<td style="text-align:right">问题二:&nbsp;</td>
		<td><select class="form-control" id="select2" onFocus="reg_problem()" onBlur="show_option()">
		<?php
			echo $pro;
		?>
		</select></td>
	...
	<tr style="line-height:45px;">
		<td style="text-align:right">答案:&nbsp;</td>
		<td><input class="form-control" type="text" style="float:right;display:block" id="a2"/></td>
	</tr>
	<tr style="line-height:45px;">
		<td style="text-align:right">问题三:&nbsp;</td>
		<td><select class="form-control" id="select3" onFocus="reg_problem()" onBlur="show_option()">
		<?php
			echo $pro;
		?>
		</select></td>
	...
	<tr style="line-height:45px;">
		<td style="text-align:right">答案:&nbsp;</td>
		<td><input class="form-control" type="text" style="float:right;display:block" id="a3"/></td>
	</tr>
	<tr style="line-height:45px;">
		<td></td>
		<td style=""><a style="width:100px;line-height:33px;background:#ec680c;display:block;vertical-align:50%;text-align:center;border-radius:5px;" href="javascript:void(0)" onClick="securityaction()"><font style="color:#ffffff">下一步</font></a></td>
	</tr>
</table>
</div>
</div>
