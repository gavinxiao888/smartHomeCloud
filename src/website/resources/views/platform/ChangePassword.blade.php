...
function reg_passwd()//验证密码
{
	...
	function passeditaction()//修改密码的动作
	{
		...
		//{
			...
			var pass = $('#pass').val();
			var pass1 = $('#pass1').val();
			alert(oldpass+"-"+pass+"-"+pass1);
			$.ajax(
			{
				type:"post",
				url:"/user/password/editaction",
				data:{oldpass:oldpass,pass:pass,pass1:pass1},
				success:function(data)
				{
					// alert(data);
					data = data.replace(/\r\n/g,'');
...
<div class="content"> 
<div class="cont-head"> 
<h4><a href="/user/account">爱悠账号</a>&nbsp;>&nbsp;<span>修改密码</span> </h4>
<h6><?php
...
<tr>
	...
	<td></td></tr>
<tr>
	<td class="span">当前密码:</td>
	<td><input style="width:300px" class="form-control" type="password" name="oldpass" id="oldpass"  onBlur="reg_passwd()"/></td>
	<td><div id="oldpassshow" style="color:red;text-align:left;"></div></td></tr>
<tr>
	<td class="span">新密码:</td>
	<td><input style="width:300px" class="form-control" type="password" id="pass"  onkeyup="pass_length('pass','showpass')" onBlur="pass()"/></td>
	<td style="font-size:12px;"><div id="showpass" style="color:red;text-align:left;">长度为8-16位，至少包含数字、字母、下划线的两种</div></td></tr>
<tr>
	<td class="span">再次输入新密码:</td>
	<td><input style="width:300px" class="form-control" type="password" id="pass1" onkeyup="pass1()"/></td>
	<td><div id="showpass1" style="color:red;text-align:left;"></div></td></tr>

<tr>
	<td class="span"></td>
	<td><button type="button" class="btn btn-warning" style="float:left;display:block;width:79px;height:30px;background:#ec680c;" onclick="passeditaction()"><font style="color:white">确认</font></button></td>
	<td></td></tr>
</table>
</div>
</div>
